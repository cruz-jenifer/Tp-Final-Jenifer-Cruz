...
import { obtenerMascotas, eliminarMascota } from '../../../store/slices/mascotasSlice';
...
export const MascotasList: React.FC = () => {
    ...
    const { mascotas, cargando, error } = useAppSelector((state) => state.mascotas);
    ...
    const handleConfirmDelete = async () => {
        if (selectedPetId) {
            await dispatch(eliminarMascota(selectedPetId));
    ...
    return (
        <div className={styles.grid}>
            {(Array.isArray(mascotas) ? mascotas : []).map((pet) => (
