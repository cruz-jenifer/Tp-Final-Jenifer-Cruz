...
import { obtenerMisTurnos, cancelarTurno, eliminarTurno, actualizarTurno } from '../../../store/slices/turnosSlice';
...
export const TurnosList: React.FC = () => {
    ...
    const { turnos, cargando, error } = useAppSelector((state) => state.turnos);
    ...
    const handleConfirmAction = async () => {
        if (selectedTurnoId) {
            ...
            if (turno?.estado === 'cancelado') {
                await dispatch(eliminarTurno(selectedTurnoId));
            } else {
                await dispatch(cancelarTurno(selectedTurnoId));
            }
    ...
    const handleEditSubmit = async (formData: EditTurnoFormData) => {
        ...
        await dispatch(actualizarTurno({ id: turnoToEdit.id, datos: updateData }));
        setIsEditModalOpen(false);
        setTurnoToEdit(null);
        dispatch(obtenerMisTurnos());
    };

    if (cargando && turnos.length === 0) return <p>Cargando turnos...</p>;
            ...
            <CardContent>
                <div className={styles.tableContainer}>
                    <table className={styles.table}>
                        ...
                        <tbody>
                            {(Array.isArray(turnosSorted) ? turnosSorted : []).map((turno) => (
                                <tr key={turno.id} className={styles.tableRow}>
                                    <td>
                                        <div className={styles.cellWithIcon}>
                                            <span className="material-icons">event</span>
                                            <span>{turno.fecha || 'N/A'}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div className={styles.cellWithIcon}>
                                            <span className="material-icons">schedule</span>
                                            <span>{turno.hora || 'N/A'}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div className={styles.cellWithIcon}>
                                    ...
                                    <td>
                                        {turno.motivo_consulta && (
                                            <ActionBtn
                                                onClick={() => { setMotivoToShow(turno.motivo_consulta || ''); setMotivoModalOpen(true); }}
                                                icon="visibility"
                                                label="Ver motivo"
                                                variant="primary"
                                            />
