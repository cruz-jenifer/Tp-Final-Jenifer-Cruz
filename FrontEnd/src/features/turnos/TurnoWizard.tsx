...
import { crearTurno, obtenerMisTurnos } from '../../store/slices/turnosSlice';
...
export const TurnoWizard: React.FC<TurnoWizardProps> = ({ onClose }) => {
    ...
    const { veterinarios } = useAppSelector((state) => state.admin);
    ...
    const handleSubmit = async () => {
        if (!formData.motivo_consulta) {
            setErrors({ motivo_consulta: 'Por favor ingrese un motivo de consulta.' });
            return;
        }
        ...
        try {
            ...
            await dispatch(obtenerMisTurnos());
            onClose();
        } catch (error: unknown) {
                    ...
                    <div className={styles.stepContent}>
                        ...
                        <div className={styles.formGroup}>
                            ...
                            <textarea
                                ...
                                onChange={(e) => {
                                    handleChange(e);
                                    if (errors.motivo_consulta) setErrors({ ...errors, motivo_consulta: '' });
                                }}
                                className={`${styles.textarea} ${errors.motivo_consulta ? styles.inputError : ''}`}
                                placeholder="Describa brevemente el motivo de la consulta..."
                            />
                            {errors.motivo_consulta && <span className={styles.errorText}>{errors.motivo_consulta}</span>}
                        </div>
                    </div>
                )}
            </div>
