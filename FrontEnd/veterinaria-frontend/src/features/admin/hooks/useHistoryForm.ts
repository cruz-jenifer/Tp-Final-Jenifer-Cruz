...
import { createHistorial, updateHistorial, type Historial } from '../../../store/slices/adminSlice';

export const useHistoryForm = () => {
    const dispatch = useAppDispatch();
    const { pets } = useAppSelector((state) => state.admin);
    ...
    const validateForm = () => {
        ...
        else if (!pets.find(p => p.id === Number(formData.mascota_id))) newErrors.mascota_id = 'Mascota no encontrada';

        if (!formData.diagnostico) newErrors.diagnostico = 'Requerido';

        if (formData.veterinario_id && !veterinarios.find(v => v.id === Number(formData.veterinario_id))) {
    ...
    const resetForm = () => {
        setFormData({ mascota_id: '', veterinario_id: '', fecha: new Date().toISOString().split('T')[0], diagnostico: '', tratamiento: '', observaciones: '' });
        setIsEditing(false);
        setEditingId(null);
        setErrors({});
    };
    ...
    const handleSubmit = async (onSuccess?: () => void) => {
        ...
        if (isEditing && editingId) {
            ...
            await dispatch(updateHistorial({ id: editingId, data: updateData }));
        } else {
            // CREAR
            const now = new Date();
            ...
            await dispatch(createHistorial(histData));
        }
