...
const VetsPage: React.FC = () => {
    ...
    return (
        <div>
            ...
            {/* FORMULARIO INLINE */}
            <div ref={formRef} className={styles.inlineFormCard} style={{ borderLeft: isEditing ? '4px solid #6384FF' : 'none' }}>
                ...
                <div className={styles.inlineFormGrid}>
                    <div className={styles.inlineFormGroup}>
                        ...
                        <input name="nombre" value={formData.nombre} onChange={handleChange} type="text" className={styles.inputField} placeholder="Ej: Juan" />
                    </div>
                    <div className={styles.inlineFormGroup}>
                        <label className={styles.inlineLabel}>Apellido</label>
                        <input name="apellido" value={formData.apellido} onChange={handleChange} type="text" className={styles.inputField} placeholder="Ej: Perez" />
                    </div>
                    <div className={styles.inlineFormGroup}>
                        ...
                        <input name="email" value={formData.email} onChange={handleChange} type="email" className={styles.inputField} placeholder="Ej: dr.juan@vet.com" disabled={isEditing} />
                    </div>
                    <div className={styles.inlineFormGroup}>
                        <label className={styles.inlineLabel}>Matrícula</label>
                        <input name="matricula" value={formData.matricula} onChange={handleChange} type="text" className={styles.inputField} placeholder="Ej: MP-12345" />
                    </div>
                </div>
                <div style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                    <button
                        ...
                        onClick={() => handleSubmit((newVet) => setSelectedVet(newVet))}
                    >
                        <span className="material-icons" style={{ fontSize: '1rem', marginRight: '5px' }}>{isEditing ? 'save' : 'add'}</span>
                        {isEditing ? 'Actualizar' : 'Agregar Veterinario'}
                    </button>
                    {isEditing && (
                        <button
                            onClick={resetForm}
